#!/usr/bin/env bash
# ==== SUBMIT VERSION of launch_alfworld_grpo_tmux.sh ====
# Bound to the submit training script: launches scripts/run_alfworld_grpo_train_submit.sh
# (which activates conda via PATH prepend instead of `conda activate`).
...
# run_alfworld_grpo_train_submit.sh. (Explicit in the command string to avoid tmux
...
for v in ENABLE_ERC ERC_CLIPPING_METHOD ERC_CLIPPING_TYPE ERC_MOMENTUM UNCERTAINTY_SCALE_KAPPA UNCERTAINTY_SCALE_MIN UNCERTAINTY_SCALE_RENORMALIZE SAFE_COMMIT_MODE SAFE_COMMIT_OMEGA SAFE_COMMIT_RECENCY SAFE_COMMIT_RECENCY_GAMMA SAFE_COMMIT_SUCCESS_THRESHOLD SAFE_COMMIT_KAPPA SAFE_COMMIT_GMAX SAFE_COMMIT_GMIN SAFE_COMMIT_RENORMALIZE SAFE_COMMIT_TEXT_GATE SAFE_COMMIT_GATE_MODE SAFE_COMMIT_CLF_ENV SAFE_COMMIT_CLF_WINS_ONLY SAFE_COMMIT_CLF_MAX_NEW \
         ...
         PLAN_FORECAST_ENABLE PLAN_FORECAST_COEF PLAN_FORECAST_K PLAN_FORECAST_GATE PLAN_FORECAST_SUCCESS_THRESHOLD PLAN_FORECAST_MAX_LENGTH PLAN_FORECAST_TARGET PLAN_FORECAST_SEQ \
         PLAN_FORECAST_COEF_ANNEAL PLAN_FORECAST_COEF_END PLAN_FORECAST_COEF_HORIZON PLAN_FORECAST_COEF_POWER PLAN_FORECAST_COEF_CUTOFF_STEP \
         PLAN_INLINE_ENABLE PLAN_INLINE_K PLAN_INLINE_STYLE PLAN_INLINE_PER_TURN PLAN_INLINE_WARMUP_STEPS THINK_REMINDER_ENABLE PLAN_FORMAT_REWARD_ENABLE PLAN_FORMAT_REWARD_COEF PLAN_FORMAT_REWARD_BASELINE PLAN_FORMAT_REWARD_CLIP PLAN_FORMAT_REWARD_PENALTY_ONLY PLAN_FORMAT_REWARD_WARMUP_STEPS \
         WMC_COEFF POLICY_LR ENTROPY_COEF KL_COEF; do
  if [ -n "${!v:-}" ]; then FWD="${FWD} ${v}=${!v}"; fi
done
echo "Forwarding overrides:${FWD:-<none>}"
tmux new-session -d -s "${TRAIN_SESSION}" \
  "cd ${ROOT} && CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} BASE_PORT=${BASE_PORT} MODEL_PATH=${MODEL_PATH} WANDB_MODE=${WANDB_MODE} PROJECT_NAME=${PROJECT_NAME} EXP_NAME=${EXP_NAME} LOG_PATH=${TRAIN_LOG}${FWD} bash ${ROOT}/scripts/run_alfworld_grpo_train_submit.sh"
