...
EXP_NAME="${EXP_NAME:-sciworld_grpo_reb_r2_${RUN_TS}}"
...
# run_sciworld_grpo_train_rebuttal.sh. (Explicit in the command string to avoid tmux
...
tmux new-session -d -s "${TRAIN_SESSION}" \
  "cd ${ROOT} && CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} BASE_PORT=${BASE_PORT} MODEL_PATH=${MODEL_PATH} WANDB_MODE=${WANDB_MODE} PROJECT_NAME=${PROJECT_NAME} EXP_NAME=${EXP_NAME} LOG_PATH=${TRAIN_LOG}${FWD} bash ${ROOT}/scripts/run_sciworld_grpo_train_rebuttal.sh"
