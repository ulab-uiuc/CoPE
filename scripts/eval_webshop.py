#!/usr/bin/env python3
"""Evaluate a local vLLM model on WebShop.
...
Features:
  ...
  * Supports WebShop environment parallelism via multiple env servers.
...
def run_trajectory(
    ...
    while not done and rounds < max_rounds:
        ...
        conversation.append({"role": "user", "content": step.state})
...
# ---------------------------------------------------------------------------
class EnvPool:
    """Thread-local WebshopEnvClients round-robining across servers."""
    ...
    def get(self) -> WebshopEnvClient:
        ...
        if client is None:
            with self._lock:
                ...
                self._counter += 1
            client = WebshopEnvClient(env_server_base=addr, data_len=1, timeout=self._timeout)
...
def load_test_ids(test_file: Path) -> list[int]:
    ...
    ids = []
    for r in rows:
        # Extract index from item_id "webshop_{idx}"
        if "item_id" in r:
...
def format_report(summary: dict[str, dict[str, float]]) -> str:
    s = summary["All"]
    return (
        f"Success Rate: {s['success']*100:.2f}%\n"
        f"Average Reward: {s['score']:.4f}\n"
        f"Count: {s['count']}"
    )

# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Evaluate WebShop with local vLLM.")
    parser.add_argument("--model-path", required=True, help="Path to local checkpoint")
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--env-addrs", default="http://127.0.0.1:36001", help="Comma-separated env URLs")
    ...
    # Resolve absolute path to model to avoid issues with subprocesses in different CWDs
