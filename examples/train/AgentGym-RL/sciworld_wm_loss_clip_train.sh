...
ppo_inner_epochs=1
world_model_coeff=5e-3

total_epoches=10

model_save_dir="saves"
mkdir -p ${model_save_dir}
exp_name="wm_loss_clip_${pure_agent_model_name}"
...
HYDRA_FULL_ERROR=1 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True WANDB_MODE=online python3 -m verl.agent_trainer.main_ppo  \
    ...
    data.train_file=../data/train/${task_name}_train.json \
    ...
    wmc_erc.enable=True \
    wmc_erc.mu_base=1.25 \
    wmc_erc.mu_exp=1.25 \
    wmc_erc.eta_wm=1.5 \
    wmc_erc.lambda_wm=1.0 \
    wmc_erc.clipping_type=global \
    wmc_erc.clipping_method=gaussian \
    wmc_erc.momentum=0.9 \
    trainer.default_local_dir=${model_save_path} \
    trainer.project_name=agentgym-sciworld \
